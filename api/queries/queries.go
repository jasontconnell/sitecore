...
const Templates = `
    select cast(rec.ID as char(36)) ID, rec.Name, cast(rec.TemplateID as char(36)) TemplateID, cast(rec.ParentID as char(36)) ParentID, cast(rec.MasterID as char(36)) MasterID,  isnull(Replace(Replace(sf.Value, '{',''), '}', ''), '') as Type, isnull(Replace(Replace(UPPER(b.Value), '{',''), '}', ''), '') as BaseTemplates, isnull(Replace(Replace(UPPER(sv.[Value]), '{',''), '}', ''), '') as StandardValuesId, isnull(sh.Value, '0') as Shared, isnull(unv.Value, '0') as Unversioned
    from Items rec
	...
	where rec.TemplateID in ('%[1]s', '%[2]s', '%[3]s', '%[4]s', '%[5]s', '%[6]s', '%[7]s')
`
